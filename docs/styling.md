...
7. **The map is a panel, not a screen.** 236px inside a scrolling page, full-bleed to the screen
   edges, with overlay controls positioned over it. Cooperative gestures: one finger scrolls the
   page, two fingers pan the map. Overlay controls sit at `z-index` 1 (re-centre, change map) and
   2 (region suggestion) — keep that ordering or the suggestion can be obscured.
...
| Picking         | `.capture-map-crosshair` (CSS arms with a centre gap, `pointer-events: none`), `.capture-map-picking` + `-copy`, `-readout`, `-actions`; the trigger is `.capture-map-pick` | The crosshair belongs to the viewport, not the ground, so it is CSS rather than a map layer.                      |
